...
[lidar_resampled,rtl] = resample(lidar_meas,t_lidar,  resampling_factor_lidar*2.2*3);
...
%% Run Kalman filter
[range_estimate, velocity_estimate, orientation_estimate] = kalman_filter_nonuniform([x6_interp,x6_1_interp,x6_2_interp],tbis_stereo_baseline, pos_var_init, vel_var_init, ori_var_init, lidar_meas_noise, radar_meas_noise, camera_meas_noise);
%% Error calculation
%% Lidar
...
hold on
%Plot stereo camera
plot(t6,sqrt(x6(:,1).^2+x6(:,2).^2));hold on
%Plot radar
plot(radar_time(:,2:end),distance(2:end,:));hold on
%PLot lidar
plot(lidar_time,ranges/1000);hold on
%plot(rtl,sqrt(lidar_resampled(:,1).^2+lidar_resampled(:,2).^2)plot(tbis, x4_interp)
xlabel('Time (s)');
ylabel('Distance (m)');
...
function [range_estimate, velocity_estimate, orientation_estimate] = kalman_filter_nonuniform(camera_meas, camera_t, pos_var_init, vel_var_init, ori_var_init, lidar_meas_noise, radar_meas_noise, camera_meas_noise)
...
num_time_steps = length(camera_t);
...
for t = 2:num_time_steps
    ...
    if t <= size(camera_meas, 1) && camera_t(t) > camera_t(t-1)
        ...
        %H_camera = [1 0 0 0 0 cos(x(7)) -sin(x(7)); 0 0.8 0 0 0 sin(x(7)) cos(x(7)); 0 0 1 0 0 0 0];
         %H_camera = [1 0 0 0 0 1 1; 0 1 0 0 0 0.8 0; 0 0 0.6 0 0 0 0];
        % H_camera = [1 0 0 0 0 0.5 0.5; 0 1 0 0 0 0.4 0; 0 0 0.3 0 0 0 0];
        H_camera = [1 0 0 0 0 0.001 0.001; 0 0.001 0 0 0 0.001 0; 0 0 0.6 0 0 0 0];
        R_camera = R(7:9, 7:9);
