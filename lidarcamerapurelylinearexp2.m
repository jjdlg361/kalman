...
%% Run Kalman filter
[range_estimate, velocity_estimate, orientation_estimate] = kalman_filter_nonuniform(x4_interp_base, [x6_interp,x6_1_interp,x6_2_interp], tbis_base,tbis_stereo_baseline, pos_var_init, vel_var_init, ori_var_init, lidar_meas_noise, radar_meas_noise, camera_meas_noise);
%% Error calculation
%% Lidar
t3 = tbis_stereo_baseline;
x3 = range_estimate;
...
hold on;
%Plot estimate
plot(tbis_stereo_baseline, range_estimate, '-o', 'LineWidth', 2);
hold on
%Plot ground truth
...
function [range_estimate, velocity_estimate, orientation_estimate] = kalman_filter_nonuniform(lidar_meas, camera_meas, lidar_t, camera_t, pos_var_init, vel_var_init, ori_var_init, lidar_meas_noise, radar_meas_noise, camera_meas_noise)
...
q_pos = get_q_pos(lidar_meas(:,1), sqrt(camera_meas(:,1).^2+camera_meas(:,2).^2), lidar_t, camera_t)
%0.4^2;
q_vel = 0.1^2;
...
x = [lidar_meas(1, 1), 0, camera_meas(1, 1), 0, 0, 0, 0]';
...
num_time_steps = max(length(lidar_t), length(camera_t));
...
for t = 2:num_time_steps
    ...
    if t <= size(camera_meas, 1) && camera_t(t) > camera_t(t-1)
        ...
        %H_camera = [1 0 0 0 0 0.001 0.001; 0 0.001 0 0 0 0.001 0; 0 0 0.001 0 0 0 0];
        R_camera = R(7:9, 7:9);
...
function q_pos = get_q_pos(lidar_meas, camera_meas,lidar_t, camera_t)
    dt=0.01;
    t = max([camera_t(1), lidar_t(1)]):dt:min([camera_t(end), lidar_t(end)]);
    xlidar = interp1(lidar_t, lidar_meas, t, 'linear', 'extrap');
    xcamera = interp1(camera_t, camera_meas, t, 'linear', 'extrap');
    mse1 = mean((xlidar - xcamera).^2);
    mse=mse1;
    if mse1 > 0.4
        q_pos = 0.4^2;
    elseif mse1 <= 0.09
